...
// ✅ CORREÇÃO: PUT notices com validação
  ...
  app.put('/api/military-personnel/:id', async (req, res) => {
    try {
      ...
      // 🔥 NOVO: Sincronização automática com oficiais de serviço
      try {
        await syncDutyOfficersAfterMilitaryUpdate(updatedPersonnel, personnel);
      } catch (syncError) {
        console.warn('⚠️ Aviso: Erro na sincronização automática:', syncError);
        // Não falhar a resposta principal por causa da sincronização
      }
