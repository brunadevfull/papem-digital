...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    // Criar militares padrão
    this.initDefaultOfficers();
      console.log('💾 MemStorage initialized'); 
  }

  private initDefaultOfficers() {
    // Oficial do Dia padrão
    const oficialDia: DutyOfficer = {
      id: 1,
      name: "Silva",
      role: "oficial_dia",
      rank: "1TEN",
    ...
    this.dutyOfficers.set(1, oficialDia);

    // Contramestre de Pernoite padrão
    const contramestre: DutyOfficer = {
      id: 2,
      name: "Santos",
    ...
    this.dutyOfficers.set(2, contramestre);

    this.currentDutyOfficerId = 3;
    console.log('🪖 Militares padrão criados: Oficial do Dia e Contramestre');
  }
...
// ✅ Notice methods - VERSÃO CORRIGIDA COMPLETA
  ...
  async createDutyOfficer(insertOfficer: InsertDutyOfficer): Promise<DutyOfficer> {
    ...
    const officer: DutyOfficer = { 
      id,
      name: insertOfficer.name,
      role: insertOfficer.role as "oficial_dia" | "contramestre_pernoite",
      rank: insertOfficer.rank,
